...
PHASE44C0_SCHEMA_FIT_AUDIT: dict[str, Any] = {
    ...
    "summary": (
        ...
        "ProviderCache and NegativeLookupCache planning still require a valid "
        "query_hash and public-safe redacted request shape from local details/raw artifacts. "
    ...
    "questions": {
        "provider_raw_redacted_cache": {
            ...
            "notes": "Store redacted normalized contract payloads only when query_hash is valid and request_shape is present/public-safe; no API keys, local paths, filenames, or raw image bytes.",
        },
        ...
        "discard_negative_result": {
            ...
            "notes": "Low-confidence manually wrong results can be cached as negative/discard outcomes without positive candidates only when query metadata is valid and public-safe.",
        },
    ...
    "c1_persistence_recommendation": {
        ...
        "write_order": [
            "ProviderCache redacted contract payload with valid query_hash/public-safe request_shape",
            ...
            "NegativeLookupCache rows for 2690, 2654, 2647 if C1 includes negative policy and valid query metadata",
        ],
        "blocked": [
