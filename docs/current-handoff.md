...
- Closeout hardening: Phase 3.6 runner now blocks DB-backed active AI jobs (`pending`/`running`/`cancelling`), hard-fails forbidden classification/translation side-effect job deltas during AI, exits nonzero on localization provider/candidate failures, marks `TagTranslationJob` failed on save/finalization exceptions, caps localization candidates to `min(--max-items, TAG_TRANSLATION_BATCH_MAX_ITEMS)`, writes partial AI failure reports before bad-chunk aborts, and rejects non-positive `ai-tag --limit` / `localize --max-items`
