...
> Last updated during Phase 4.4-C0 provider-neutral evidence contract foundation (2026-05-27).
...
| **Branch** | `main` includes PR #72/#73/#74/#75/#76/#77/#78; current work branch is `codex/phase-4.4-c0-provider-evidence-contract` |
...
| **Phase 3.8c (PR #54)** | [PR #54](https://github.com/kyloris0660/AnimeLocalBooru/pull/54) merged - medium +1000 candidate preflight with temporal stratified selection; dry-run only |
| **Phase 3.8d-I1 (PR #55)** | [PR #55](https://github.com/kyloris0660/AnimeLocalBooru/pull/55) merged - iCloud / Windows Cloud Files ingestion reliability incident hardening; Phase 3.8d execute blocked |
| **Phase 3.8d-I2 (PR #56)** | [PR #56](https://github.com/kyloris0660/AnimeLocalBooru/pull/56) merged - source ingestion gate unification; no execute/resume yet |
| **Phase 3.8d-I3 (PR #57)** | [PR #57](https://github.com/kyloris0660/AnimeLocalBooru/pull/57) merged - partial staging cleanup dry-run plus controlled hydration/read-probe and same-bucket backfill policy; no execute/resume yet |
| **Phase 3.8d-I4a (PR #58)** | [PR #58](https://github.com/kyloris0660/AnimeLocalBooru/pull/58) merged - controlled partial staging cleanup executor support and tests; no real cleanup performed |
| **Phase 3.8d-I4b (PR #59)** | [PR #59](https://github.com/kyloris0660/AnimeLocalBooru/pull/59) merged - actual partial staging cleanup completed: `97` files / `340,159,586` bytes deleted from the dedicated staging target; DB/source/app storage unchanged |
| **Phase 3.8d-I5 (PR #60)** | [PR #60](https://github.com/kyloris0660/AnimeLocalBooru/pull/60) merged - controlled read-probe / hydration audit; sample gate failed for rows `98` and `881`, so full recall verification and Phase 3.8d execute remain blocked |
| **Phase 3.8d-I5b (PR #61)** | [PR #61](https://github.com/kyloris0660/AnimeLocalBooru/pull/61) merged - targeted retry for rows `98` and `881`; both rows failed bounded prefix/full-read retry with `cloud_hydration_failed`, backfill remained dry-run only, and Phase 3.8d execute stayed blocked |
| **Phase 3.8d-I5c (PR #62)** | [PR #62](https://github.com/kyloris0660/AnimeLocalBooru/pull/62) merged - replacement rows `1029` and `1041` validated by controlled full-read, local backfilled selected manifest generated with `selected_total=1000`, rows `98` and `881` recorded in deferred cloud recovery ledger with per-run final state; no staging copy or DB import |
| **Phase 3.8d-I6 (PR #63)** | [PR #63](https://github.com/kyloris0660/AnimeLocalBooru/pull/63) merged - cloud-aware copy policy explicitly enabled; staging copy completed with item-level failures: `994` files / `3,063,523,992` bytes staged, `6` rows failed with `cloud_network_unavailable`, failure budget not exceeded; full `1000` DB import remains blocked |
| **Phase 3.8d-I7 (PR #64)** | [PR #64](https://github.com/kyloris0660/AnimeLocalBooru/pull/64) merged - partial import from the I6 item ledger staged-success set completed for `994` rows; failed rows `799`, `839`, `922`, `970`, `971`, and `972` excluded; classification processed `994` with `0` failures; AI tagging processed `967` anime/unknown rows with `0` failures; initial controlled localization translated `200` general/meta tags, then closeout continuation translated the remaining `370` eligible general/meta tags with `0` failures and `0` remaining general/meta missing translations; manual validation accepted the medium pilot at a practical level |
...
| **Phase 4.4-B1V ([PR #78](https://github.com/kyloris0660/VIOLET/pull/78))** | Merged - manual validation and SauceNAO metadata extraction audit; high-confidence matches validated correct, low-confidence matches discarded, character metadata confirmed present in high-confidence API data after a two-item metadata-preservation re-query |
| **Phase 4.4-C0 (current PR)** | Active - provider-neutral reverse-search evidence contract, SauceNAO mapper foundation, non-mutating schema-fit audit, and focused tests; no provider calls, uploads, DB writes, migrations, confirmed assignments, Entity creation, localization, Entity Resolver, or similarity/clustering |
...
Phase 4.4-C0 defines the provider-neutral evidence contract for reverse-search provider outputs and maps the existing SauceNAO B1/B1V facts into it without mutation. The contract includes `ProviderQuery`, `ProviderRunOutcome`, `SourceMatch`, `ExtractedProviderMetadata`, `EvidencePersistencePlan`, and planned nullable-entity candidates. The SauceNAO mapper treats `2687` and `2670` as manually validated strong exact/near-exact source evidence with raw provider artist/work/character metadata preserved and `localization_status=pending`; it treats `2690`, `2654`, and `2647` as discarded low-confidence results with no positive evidence/candidate plan. Closeout hardening rejects fabricated, placeholder, malformed, or missing query hashes; accepts only existing 64-hex sha256 query hashes or `sha256:<64 hex>` as `present_valid`; blocks all positive persistence planning unless provider provenance is ready; requires public-safe source identifier fields before persistence planning; normalizes secret-like and forbidden privacy key variants before public serialization; rejects non-finite numbers before public JSON/report emission; marks non-JSON request shapes invalid instead of crashing; decodes URL-encoded strings before local-path/secret scanning; and makes discard/wrong/unrelated/metadata-not-useful manual signals dominate conflicting keep/correct fields. A reduced public summary may still expose `non_persistable_source_match=true` when the source match itself is useful, but it cannot plan ProviderCache, EntityEvidence, or MediaEntityCandidate rows. C1 should consume only persistence-ready plans from local details/raw provider artifacts with valid query hashes, public-safe JSON redacted request shapes, and public-safe source identifiers; reduced public summaries are not enough for provider provenance readiness. The schema-fit audit concludes Phase 4.1 tables are `sufficient_with_json_payload` for a narrow C1 persistence pass, but first-class match/manual-validation/localization columns remain follow-up design. C0 performs no provider call, upload, DB write, DB migration, ProviderCache/NegativeLookupCache/EntityEvidence/MediaEntityCandidate/MediaEntityAssignment write, automatic Entity creation, confirmed assignment, media_tags mutation, TagTranslation mutation, localization execution, Entity Resolver, similarity/clustering, source/iCloud mutation, or app-managed storage mutation. Public report: `docs/reports/phase-4.4c0-provider-neutral-evidence-contract.md`; summary JSON: `docs/reports/phase-4.4c0-provider-neutral-evidence-contract-summary.json`.
...
- Phase 4.4-C0 provider-neutral contract: `2687` and `2670` map to `match_class=exact_or_near_exact`, `evidence_strength=strong`, raw artist/work/character metadata preserved, `localization_status=pending`, and C1 candidate/evidence plans with no DB write in C0 when real query metadata and source identifiers are present. `2690`, `2654`, and `2647` map to `match_class=discarded`, `evidence_strength=discard`, no positive candidates, and optional future negative-cache persistence only if real query metadata exists. Schema fit is `sufficient_with_json_payload`.
...
- Phase 4.4-C0 confirms the next safe implementation path is Phase 4.4-C1 validated high-confidence evidence persistence for `2687` and `2670`, using the provider-neutral contract. C1 should still avoid confirmed assignments, automatic Entity creation, localization execution, provider reruns, uploads, and broad sample expansion.
- Later second-provider scouting should be Phase 4.4-D0 and must map into the same contract instead of introducing a separate DB write path.
...
3. **Chinese reporting** — Final delivery reports and stage summaries must be written in Chinese (zh-CN). Section headings must also be Chinese. Technical identifiers remain English.
...
- **force_suggestions**: Option to write all AI tags as suggestions for targeted correction/review workflows (regardless of confidence); this must not imply exhaustive manual processing of every suggestion.
