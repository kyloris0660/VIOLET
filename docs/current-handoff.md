...
- Phase: `SCV2-FL1-I1` — Read-Only Inventory Planning And Synthetic Foundation.
...
- Approved planning HEAD: `db90457d51a39b5dc930afc2a92a6ef3139a2760`; route scope: `FL1-I1 planning and synthetic read-only inventory implementation only`.
...
- Allowed: owner audit and code review of Draft PR #142 and its public-safe synthetic inventory contract evidence; local bounded scanning of newly created synthetic fixtures inside temporary sandboxes with in-memory manifest output only; focused and full non-E2E validation plus bounded review-driven corrections inside the FL1-I1 synthetic foundation scope.
- Forbidden: existing database creation, connection, read, comparison, write, import, mutation, replay, derivation, cleanup, reset, truncate, drop, or repair; production database, production storage, production source root, production library, or any other production access; real source or iCloud root listing, inventory execution, read, hashing, hydration, copy, move, rename, deletion, or mutation; app storage creation or write, import execution, classification execution, AI tagging execution, localization, graph/search execution, or background worker start; provider execution, Pixiv, gallery-dl, Provider-2, LLM execution, model download, media download, thumbnail request, or network access; Entity, EntityAlias, confirmed assignment, user truth, source truth, or provider-derived media_tags write; Stable Replay or authoritative evidence import, replay, or reuse; Ready transition, merge, direct main push, force-push, or any FL1 stage beyond owner audit of the FL1-I1 synthetic foundation.

## Next Action

- Required checkpoint: `owner_audit_of_draft_pr_142_then_separate_exact_real_source_scope_decision_before_any_scan`.
