...
> Last updated during Phase 4.4-C1-HF1 DB write gate hotfix (2026-05-28), after PR #81 was merged.
...
- Phase 4.4-C1-HF1 hotfix is the current branch work: enforce `EvidencePersistencePlan.db_write_allowed` in the durable persistence service and make the C1 runner explicitly promote only approved validated plans to writable after all C1 gates pass. Expected DB impact is zero new rows; existing accepted C1 rows remain accepted unless later validation proves a data correction is needed. Public report: `docs/reports/phase-4.4c1-db-write-gate-hotfix.md`.
...
Near-term route after Phase 4.4-C1-HF1:
