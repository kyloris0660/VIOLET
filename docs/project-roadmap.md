...
- After PR #86 reviewer hardening, preview host allowlisting fetched Pixiv preview/reference images for 4 of 5 and blocked 1 unexpected/static preview-host candidate without fetching it. No cookies, login, browser automation, Referer spoofing, or original image download occurred. Exact preview URLs and fetched files remain ignored local artifacts only.
- Local correspondence verification after hardening produced `1` `auto_verified_high_confidence`, `3` `auto_rejected_mismatch`, and `1` `preview_fetch_blocked` under P1 pilot thresholds. These are pilot thresholds, not production thresholds, and do not create confirmed evidence.
- Generated a local ignored manual validation pack for the 4 non-auto-verified rows: Markdown/CSV validation sheet plus HTML/Markdown contact sheet under `.local_manifests/phase-4.4p1-pixiv-manual-validation-*`.
...
2. Before P2, manually validate the 4 non-auto-verified P1 rows (`3` mismatch plus `1` preview-host-blocked row). If most are human visual matches, fix threshold/preview/page-index handling instead of lowering filename-prior confidence; if true mismatches exist, narrow confidence by filename form.
