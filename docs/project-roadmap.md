...
- Closeout hardening rejects missing/fabricated query hashes, blocks ProviderCache planning when `query_hash` or `request_shape_redacted` is missing, rejects secret-like public payload keys by normalized pattern, and requires source identifiers before high-confidence manually validated rows become strong evidence.
- C1 should use local details/raw provider artifacts with real query hashes, redacted request shapes, and source identifiers; reduced public summaries are not enough for ProviderCache persistence readiness.
