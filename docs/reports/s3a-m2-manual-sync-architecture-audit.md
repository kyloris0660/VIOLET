...
|---:|---|---|---|---|---|
| 0 | App-media-backed follow-up from `not_processed_budget_stop` / imported recovery | Plan follow-up DB pass | Counts as downstream follow-up | No | Fixes run #16/#17 recovery; cannot be hidden by filesystem, source-walk health, mtime, or stable skip. Priority `0` is a real priority and must not be coerced to default priority. |
...
| App-media follow-up vs filesystem metadata walk | `conflict-fixed-now` | Follow-up does not require source readability and remains selected even when filesystem fallback records walk errors; import candidates may be blocked/partial separately. |
...
| App-media follow-up vs cap-limited batch | `conflict-fixed-now` | Follow-up consumes actionable cap and has higher priority than imports for recovery rows; priority `0` is preserved rather than falling back to default priority. |
...
- Can cap be consumed before higher-priority work? Previously yes for stale backlog/existing media, and priority `0` was accidentally coerced to default priority by a falsy `or 100` sorter. Fixed by actionable cap semantics, stale backlog repair, and priority-preserving follow-up sorting.
