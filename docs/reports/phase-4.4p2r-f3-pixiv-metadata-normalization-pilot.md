...
- Sample gate closeout: seed selection now honors requested capacity before adding diversity seeds; `--sample-size 1` and `--sample-size 2` cannot over-select.
...
- Automated category lookup: `{"cache_hit_count": 18, "cache_miss_count": 0, "cache_write_count": 0, "cache_write_enabled": true, "enabled": true, "external_request_budget": 600, "hard_lookup_limit": 500, "hit_count": 18, "lookup_delay_seconds": 0.05, "lookup_error_count": 0, "lookup_limit": 200, "lookup_source": "multisource_tag_category_lookup_v1", "lookup_sources_attempted": ["danbooru_tags_api_v2", "danbooru_tag_aliases_api_v2", "safebooru_tags_xml_api_v1"], "lookup_timeout_seconds": 5, "negative_cache_hit_count": 347, "not_found_count": 115, "provider_block_reason": null, "provider_blocked": false, "request_budget_exhausted": false, "request_count": 0, "resolved_namespace_counts": {"artist": 1, "character": 1, "copyright": 3, "general": 13}, "source_cache_hit_counts": {"danbooru_tags_api_v2": 17, "safebooru_tags_xml_api_v1": 1}, "source_cache_miss_counts": {}, "source_cache_write_counts": {}, "source_hit_counts": {"danbooru_tags_api_v2": 17, "safebooru_tags_xml_api_v1": 1}, "source_lookup_error_counts": {}, "source_negative_cache_hit_counts": {"danbooru_tag_aliases_api_v2": 116, "danbooru_tags_api_v2": 116, "safebooru_tags_xml_api_v1": 115}, "source_not_found_counts": {"safebooru_tags_xml_api_v1": 115}, "source_request_counts": {}, "unique_normalized_tag_count": 133}`.
- Cache hit semantics: resolved cache hits now apply active manual overrides as the effective namespace while preserving the provider namespace separately; manual overrides remain candidate-routing metadata, not confirmed entity truth.
- Retry semantics: future `retry_after` values suppress same-source `lookup_error` retries; expired cooldown and stale `not_found` rows remain retryable.
- Request accounting: blocked, failed, alias consequent, and other multi-request lookup attempts are counted against `request_count`, `source_request_counts`, and the external request budget.
- Provider block handling: once a provider/source blocks the current run, that source is skipped for remaining tags and the provider-wide block is not written as per-tag cache truth.
- Local trusted-provenance gate: local Tag categories only promote `artist` / `character` / `copyright` entity namespaces when backed by trusted local tag provenance; AI-generated or unknown-provenance proper-noun tag categories remain non-truth candidate signals.
...
- Path validation timing: private output paths and public report paths are validated before gallery-dl/network calls, external lookup, DB migration, DB cache writes, and raw/private artifact writes.

## Recommended Next Phase

- Decision: `taxonomy_alias_knowledge_base_stage_required`.
- Reason: `multisource_lookup_cache_proof_completed_but_category_coverage_target_not_reached`.
- DB persistence: `do_not_persist_LocalSourceHint_or_PixivMetadata_yet`.

## Closeout Conclusion

- Multisource lookup/cache proof: `completed`.
- Classification target reached: `False`.
- Persistence recommendation: `do_not_persist_LocalSourceHint_or_PixivMetadata_yet`.
- EntityCandidate recommendation: `blocked_until_category_coverage_and_provenance_improve`.
- Next-stage recommendation: `taxonomy_alias_knowledge_base_stage_required`.
- Scope note: do not continue expanding APIs or provider sources inside PR #90; unresolved-tag reduction should move to a separate taxonomy / alias knowledge-base stage.
- Bounded closeout fixes: alias cache preserves antecedent keys; cache hits apply manual overrides; retry_after cooldown is respected; failed multi-request lookups count against budget; private and public paths fail closed before side effects; dry-run/skip-network remain side-effect-free.
- Final closeout fixes: sample seed selection respects requested size; local entity-namespace tag categories require trusted provenance; provider blocks stop further same-source requests.
