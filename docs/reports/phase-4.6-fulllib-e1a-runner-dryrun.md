...
E1a validates the intended production DB configuration without connecting or writing. The accepted production database name is `violet_library_prod`. Production storage is validated as an explicit, non-overlapping app-managed storage root; E1a does not create directories or write files there.
