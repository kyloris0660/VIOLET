...
- Unknown-role deterministic / LLM candidates: `4455` / `104`.
- Unknown-role materialized / review-only / unauthorized materialized: `0` / `4559` / `0`.
- Unknown-role corroboration distribution: `{'manual_confirmed_count': 0, 'independent_non_ai_sources_count': 486, 'explicit_compatible_context_count': 0, 'cross_script_observed_count': 78, 'strong_cross_script_count': 0}`.
...
- Concepts total/active/needs_review: `2767` / `1064` / `1703` -> `5408` / `1093` / `4315`.
- Gap total: `4443` -> `9363`.
...
- Search aggregate after: `{'groups_tested': 10, 'seeds_tested': 58, 'matched_seeds': 42, 'unmatched_seeds': 16, 'symmetric_groups': 0, 'asymmetric_groups': 10, 'asymmetry_reason_buckets': {'concept_split': 33, 'needs_review_not_included_in_active_search': 7, 'unmatched_alias': 16, 'missing_alias_or_unmatched_seed': 10, 'active_only_vs_needs_review_contrast': 4}, 'unmatched_aliases_count_as_asymmetry': True, 'media_result_overlap_metrics': {'pairwise_jaccard_count': 32, 'average_pairwise_jaccard': 0.1552, 'min_pairwise_jaccard': 0.0}}`.
- Search symmetry: `0` / `10` -> `0` / `10`.
- Unmatched seeds: `16` -> `16`.
- Average pairwise Jaccard: `0.3752` -> `0.1552`.
- Metric deltas: `{'scalar_delta': {'concept_total': 2641, 'active': 29, 'needs_review': 2612, 'superseded': 0, 'gap_total': 4920}, 'gap_bucket_delta': {'cjk_alias_without_english_romaji_sibling': 200, 'danbooru_parenthetical_without_cjk_sibling': 2050, 'high_frequency_source_tag_or_name_unlinked': 0, 'identity_tag_present_no_source_concept_alias': 0, 'needs_review_cluster_with_no_active_alias_path': 2267, 'same_display_name_split_across_contexts': 201, 'same_normalized_alias_key_split_across_multiple_concepts': 202, 'source_assertion_present_not_connected': 0, 'source_name_present_no_source_concept_alias': 0, 'source_tag_present_no_source_concept_alias': 0}, 'search_delta': {'matched_seeds': 0, 'unmatched_seeds': 0, 'symmetric_groups': 0, 'asymmetric_groups': 0}}`.

## Quality

- Existing / compatible proof-grade / semantic-prior same decisions: `2072` / `305` / `1767`.
- Compatible same retained / intentionally constrained / unexplained / missing-signal: `22` / `283` / `0` / `0`.
- Compatible same accounting: `305 = 22 + 283 + 0`; complete: `True`.
