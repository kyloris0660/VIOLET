...
| Current SCV2-FL1-I2 synthetic pre-real hardening | Repository-venv identity and isolated-import preflight; `& "$PY" scripts/check_documentation_state.py --check`; focused G0 trusted-Git/path/untracked/per-parent history regressions; Windows same-handle live-temp feasibility; POSIX fail-closed, worker, recovery, contract, redaction, SourceGate/cloud/scanner/I1 compatibility suites; then full non-E2E. Parse tracked JSON and run mainline/cached diff, forbidden-path, privacy, secret, and corruption scans. Only adversarial newly created temporary fixtures are authorized. Server, browser/E2E, DB, real source/iCloud, provider/LLM/model/media, Stable Replay, and production remain forbidden. |
