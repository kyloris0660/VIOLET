...
| `tests/test_dynamic_library_sync.py` | Phase 4.7-S1 DB-backed dynamic sync tables, idempotent update checks, pending counts, threshold policy, source-root safety, readiness APIs, AI/localization readiness, localization gap summary, and proper-noun safeguards |
...
| `tests/e2e/admin-content.spec.ts` | `VIOLET_RUN_REAL_E2E=1` | Admin Content section navigation, Dynamic Library Sync panel default-off controls |
