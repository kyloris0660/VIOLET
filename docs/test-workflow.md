...
| SCV2-FL1 isolated Dev/Test planning | Python identity; focused documentation-state/roadmap/contract-boundary tests; all tracked JSON parse; `scripts/check_documentation_state.py --check`; public redaction; `git diff --check`. No server/browser/database/source/inventory/import/provider/LLM/media/classification/AI-tagging/graph/search/localization execution. |
| Future SCV2-FL1 implementation/execution | Requires separate owner approval, strict-test DB and local test-storage isolation, source-scope authorization, exact inventory denominator, per-item ledgers, offline model preflight, a registered FL1 executable contract, mutation allowlist/denylist, failure budgets, redaction, and manual stop points. Production remains a separate later decision. |
