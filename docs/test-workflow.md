...
| Current SCV2-FL1-I2 governance/planning PR | Repository-venv identity preflight when Python runs; `& "$PY" scripts/check_documentation_state.py --check`; `& "$PY" -m pytest tests/test_current_handoff_freshness.py tests/test_phase45_doc1_documentation_state.py tests/test_pd1a_mainline_governance.py -q`; hostile `GIT_DIR`, `GIT_WORK_TREE`, mixed-case `GIT_*`, injected `core.fsmonitor`, exact frozen-tree, and tree-mismatch regressions; parse every tracked JSON; `git diff origin/main...HEAD --check`; `git diff --cached --check`; changed-path, privacy/redaction, forbidden-runtime/I1/DB/UI/provider scope, and branch-equality audits. External source/provider/model/media data-plane network operations remain zero; authorized Git/GitHub governance control-plane fetch/push/PR/review operations are allowed and have occurred. Raw full non-E2E, server, browser/E2E, DB, source/iCloud, model/provider/LLM, and media validation are not required or authorized. |
