...
| Current SCV2-FL1-P1 safety/ledger implementation | `& "$PY" -m pytest tests/test_scv2_fl1_p1_foundation.py tests/test_phase_contracts.py -q`; contract/isolation/containment, normalized production/unknown rejection, DB/source/storage separation, parent-qualified membership plus logical-target idempotency, restart and explicit mutation-outcome reconciliation, per-item/global failure-budget separation, stale-writer rejection, event-derived forbidden-operation counts, exact executed stages, final-tree-bound acceptance evidence, manual-stop, mutation default-deny, and synthetic-only tests; then full `& "$PY" -m pytest tests/ -q`, tracked JSON, documentation-state, redaction, and diff/path audits. No server/browser/production/existing-DB/real-source/inventory/import/provider/LLM/media/Stable-Replay/classification/AI-tagging/graph/search/localization execution. |
| Current SCV2-FL1-I1 synthetic inventory foundation | `& "$PY" -m pytest tests/test_scv2_fl1_i1_inventory.py tests/test_phase_contracts.py -q`; exact synthetic identity/containment, finite item/byte bounds, read-only descriptors, symlink/race rejection, immutable source snapshots, stable private membership, content duplicates, cloud/unreadable terminal states, denominator equations, import-deferred boundary, and aggregate-only redaction; then documentation-state, tracked JSON, full non-E2E, redaction, diff/path audits. No real source scan, DB/app-storage write, import, production, provider/LLM/media/network, Stable Replay, classification/AI-tagging, graph/search, or localization execution. |
