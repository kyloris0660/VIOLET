...
| Current SCV2-FL1-P1 safety/ledger implementation | `& "$PY" -m pytest tests/test_scv2_fl1_p1_foundation.py tests/test_phase_contracts.py -q`; contract/isolation/containment, parent-qualified membership plus content-fingerprint deduplication, restart/idempotency, explicit interrupted-mutation reconciliation, per-item/global failure-budget separation, stale-writer rejection, manual-stop, mutation default-deny, and synthetic-only tests; then full `& "$PY" -m pytest tests/ -q`, tracked JSON, documentation-state, redaction, and diff/path audits. No server/browser/production/existing-DB/real-source/inventory/import/provider/LLM/media/Stable-Replay/classification/AI-tagging/graph/search/localization execution. |
