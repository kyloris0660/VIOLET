...
| Current SCV2-FL1-P1-R1 late-review remediation | `& "$PY" -m pytest tests/test_scv2_fl1_p1_foundation.py tests/test_phase_contracts.py tests/test_phase45_doc1_documentation_state.py -q`; repository-verified PR/squash evidence with relevant-worktree cleanliness, actual-process Python identity, canonical private-ledger-derived full public projection, logical-target idempotency, outcome-unknown reconciliation, per-item invocation attribution, fixed-schema private-ledger-derived five-scenario failure-budget/manual-stop evidence, independent COMMITTED/UNKNOWN/NOT_COMMITTED interruption/restart scenarios, recursive complete-summary public redaction, required-stage enforcement, canonical source/database/storage disjointness, per-item/global budget separation, restart, stale-writer, manual-stop, default-deny, and governance regression tests; then full `& "$PY" -m pytest tests/ -q`, Python compilation, all tracked JSON, documentation-state, contract CLI with `--repo-root`, `--expected-python "$PY"`, `--runtime-ledger`, `--failure-budget-scenarios`, and `--reconciliation-scenarios`, simulated PR and trusted squash carry-forward (documentation post-squash uses `--implementation-evidence`), redaction, diff, and changed-path audits. Audit-ready PASS must fail with absent, partial, wrong-interpreter, or dirty-worktree context. Generic callbacks prove synthetic harness attribution only; phase non-action status remains an operator attestation. Automated positive owner/merge/route authority is disabled pending `OWNER_AUTHORITY_GATE`. Browser validation: N/A - no UI/runtime-server changes. No FL1-I1 scanner/contract/tests or server/production/existing-DB/real-source/inventory/import/provider/LLM/media/Stable-Replay/classification/AI-tagging/graph/search/localization execution. |
...
| S3A-M1 manual sync execute + UI | `& "$PY" -m pytest tests/test_dynamic_library_sync.py tests/test_s3a_m1_manual_sync_execute.py tests/test_phase_contracts.py -v`; launcher `npm test`; `& "$PY" scripts/check_phase_contract.py --contract s3a_m1_manual_sync_execute_contract_v1 --summary docs/reports/s3a-m1-manual-sync-execute-summary.json`; `& "$PY" scripts/check_phase_contract.py --contract public_redaction_contract_v1 --summary docs/reports/s3a-m1-manual-sync-execute-summary.json`; real browser validation on a controlled `VIOLET_ENV=test` server for Admin Dynamic Library Sync plan/confirmation/execute controls. No production execute/import/classification/AI/localization writes, no provider/LLM/SourceConcept/Entity work, no source/iCloud mutation, no automatic/scheduled/startup sync |
| Phase 4.7-S3 incremental sync automation / hardening | Requires S2 baseline validation, explicit opt-in design for any automatic production writes, retry/backfill/resume tests, scheduler/threshold tests, and browser validation for UI changes. Auto-sync must remain visibly configurable and off unless explicitly enabled |
...
| `tests/test_dynamic_library_sync.py` | Phase 4.7-S1/S2G DB-backed dynamic sync tables, idempotent update checks, pending counts, threshold policy, source-root safety, readiness APIs, AI/localization readiness, localization gap summary, proper-noun safeguards, and manual dry-run planner |
| `tests/test_s3a_m1_manual_sync_execute.py` | S3A-M1 guarded manual sync execute gates, stable public-safe plan hash, default-disabled execute, stale replan rejection, and dev/test import without AI/LLM side effects |
...
| `tests/e2e/admin-content.spec.ts` | `VIOLET_RUN_REAL_E2E=1` | Admin Content section navigation, Dynamic Library Sync panel default-off controls, and S3A-M1 manual plan/execute UI affordances when in scope |
