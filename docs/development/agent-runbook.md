# V.I.O.L.E.T. Detailed Agent Runbook (Archived From Root AGENTS.md)

> Detailed operating guidance extracted by DOC-GOV-02. The concise root
> `AGENTS.md` remains the authoritative entrypoint and hard-rule index.

## Project context

本任务的计划、进度、PR 和交付报告使用简体中文。当前明确授权优先于历史
阶段的限制；在已授权范围内持续完成实现、迁移排练和验收，不重复索要确认。
生产 Pixiv A1 的具体授权与迁移方案见 `docs/plans/production-pixiv-a1.md`。
该授权不扩展到其他阶段，不授权 merge、main push 或额外触发 reviewer。
正式材料使用项目所有者（kyloris0660，操作授权）、项目负责人（本次路线规划
ChatGPT 会话，路线与复审）、执行代理（当前 Codex，实现与验证）、审查者
...
1. **Never use the global/system Python** (`python.exe` from PATH or any interpreter outside the project venv). This includes `pip install` — never install packages into the global Python.
...
- **GitHub CLI:** use `gh` from PATH. If unavailable, locate the installed binary read-only before reporting it missing.
- Do not say "gh is unavailable" until both PATH and the platform's standard installation location have been checked.
