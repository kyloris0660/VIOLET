#!/usr/bin/env python3
"""Issue the fixed same-HEAD local validation receipt for SCV2-PX1."""
...
from scripts.phase_contracts.scv2_px1_contract import (  # noqa: E402
    Scv2Px1EvidencePaths,
    load_px1_evidence_artifacts,
)
from scripts.scv2_px1_validation_receipt import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run canonical focused PX1 tests and issue a same-HEAD receipt."
    )
    parser.add_argument("--evidence-dir", required=True)
    args = parser.parse_args(argv)
    try:
        evidence = load_px1_evidence_artifacts(
            Scv2Px1EvidencePaths(Path(args.evidence_dir)),
        ...
        json.dumps(
            {
                ...
                "owner_authority_machine_verifiable": receipt[
