...
class ContractRepositoryContext:
    ...
    scv2_px1_evidence: object | None = None
    ...
    for check_name in contract.custom_checks:
        if check_name == 'production_import_recovery':
            ...
            try:
                ...
                if repository_context is None or not repository_context.production_import_recovery_evidence:
                    raise ValueError('import_recovery_private_evidence_required')
                actual = derive_result(Path(repository_context.production_import_recovery_evidence), repository_context.repo_root)
                if dict(summary) != actual:
                    raise ValueError('import_recovery_projection_mismatch')
            except (ValueError, OSError, KeyError, TypeError) as exc:
                result.fail('import_recovery_evidence_invalid', str(exc))
            continue
        if check_name == 'production_pixiv_a1':
            from scripts.check_production_pixiv_a1 import check_public_result, derive_result
            try:
                check_public_result(summary)
                if repository_context is None or not repository_context.production_pixiv_a1_evidence:
                    raise ValueError('a1_private_evidence_required')
                actual = derive_result(Path(repository_context.production_pixiv_a1_evidence), repository_context.repo_root)
                if dict(summary) != actual:
                    raise ValueError('a1_evidence_projection_mismatch')
            except (ValueError, OSError, KeyError, TypeError) as exc:
                result.fail('a1_evidence_invalid', str(exc))
            continue
        ...
        if check_name == "scv2_fl1_i2_pre_real_hardening":
            ...
            continue
        if check_name == "scv2_px1_pixiv_metadata_consolidation":
            from .scv2_px1_contract import check_scv2_px1_contract

            check_scv2_px1_contract(
                contract,
                summary,
                result,
                repository_context=repository_context,
            ...
            continue
        if check_name == "scv2_px2_deterministic_pixiv_clustering":
            from .scv2_px2_contract import check_scv2_px2_contract

            check_scv2_px2_contract(
                contract,
                summary,
                result,
                repository_context=repository_context,
            ...
            continue
        if check_name == "scv2_px3_pixiv_product_integration":
            from .scv2_px3_contract import check_scv2_px3_contract

            check_scv2_px3_contract(
                contract,
                summary,
                result,
                repository_context=repository_context,
