#!/usr/bin/env python3
"""Issue the fixed same-HEAD local validation receipt for SCV2-PX2."""
...
from scripts.phase_contracts.scv2_px2_contract import (  # noqa: E402
    Scv2Px2EvidencePaths,
    load_px2_evidence_artifacts,
)
from scripts.scv2_px2_validation_receipt import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run canonical focused PX2 tests and issue a same-HEAD receipt."
    )
    parser.add_argument("--evidence-dir", required=True)
    args = parser.parse_args(argv)
    try:
        evidence = load_px2_evidence_artifacts(
            Scv2Px2EvidencePaths(Path(args.evidence_dir)),
