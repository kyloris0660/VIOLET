#!/usr/bin/env python3
"""Run the repository-owned synthetic SCV2-PX3 product vertical slice."""
...
def _task_runtime_environment() -> Iterator[None]:
    ...
    with tempfile.TemporaryDirectory(
        prefix="violet-scv2-px3-runtime-storage-"
    ) as runtime_storage:
        os.environ.update(
            {
...
def _run(workspace: Path, *, retain_evidence: bool) -> dict[str, object]:
    ...
    px1_summary, px2_summary, result = (
        run_repository_synthetic_pixiv_product_integration(workspace=workspace)
    )
    if retain_evidence:
        write_pixiv_product_evidence(
            workspace,
            px1_summary=px1_summary,
            px2_result=px2_summary,
            px3_result=result,
        )
    return result
...
def main(argv: list[str] | None = None) -> int:
    ...
    with _task_runtime_environment():
        if args.evidence_dir is not None:
            ...
            with tempfile.TemporaryDirectory(prefix="violet-scv2-px3-") as temporary:
