...
def extract_sc1_pipeline_evidence(root: Path = ROOT) -> dict[str, Any]:
    ...
    public_llm = public_summary.get("llm_adjudication", {})
...
def classify_severity(fidelity: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if conclusion == "full_chain_faithfully_rerun":
        return {"severity": "S3", "label": "documentation/test weakness only", "rationale": "Full-chain rerun evidence is present."}
...
def build_summary() -> dict[str, Any]:
    ...
    remediation_decision = {
        ...
        "rationale": (
            ...
            "bounded LLM pair adjudication was disabled. A1 route approval must stay provisional until R1R and a "
            "fresh A1 route audit are complete."
        ),
        "r1r_plan": [
    ...
    impacts = {
        ...
        "a1": {
            ...
            "route_approval_remains_provisional": True,
...
- Severity: `{severity['severity']}` - {severity['label']}
...
def write_outputs(summary: dict[str, Any], output_dir: Path, write_public_report: bool) -> dict[str, Any]:
    ...
    redaction = scan_public_text_for_leaks(report_text)
